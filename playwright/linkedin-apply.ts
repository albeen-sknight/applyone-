...
async function main() {
  ...
  const runDir = await createRunDir("linkedin");
  ...
  console.log("ApplyOne LinkedIn assisted dry-run");
  console.log(`Job: ${data.job.title} at ${data.job.company}`);
  console.log("No LinkedIn credentials are stored or entered. The script stops before final submission.");
  ...
  try {
    ...
    if (await page.getByText(/sign in|inicia sesi|login/i).count()) {
      throw new Error("LinkedIn login appears required. Log in manually in the browser; credentials are not automated.");
    }

    const easyApply = page.getByRole("button", { name: /easy apply|solicitud sencilla|solicitar/i });
    if ((await easyApply.count()) > 0) {
      console.log("LinkedIn Easy Apply entry detected; opening modal for safe fields only.");
      await easyApply.first().click();
      await humanDelay();
      await screenshot(page, runDir, "02-easy-apply-opened", summary);
    } else {
      throw new Error("LinkedIn Easy Apply was not visible. Manual check required.");
    }
    ...
    if (options.confirmSubmit) {
      console.log("--confirm-submit was provided, but Phase 4B LinkedIn script remains review-only.");
    ...
    if (data.application?.id) {
      await updateApplication(env.apiBase, data.application.id, {
        status: "ready_to_apply",
        notes: `LinkedIn assisted dry-run stopped before submit. Fields: ${summary.fieldsFilled.join(", ") || "none"}.`
      });
    }
  } catch (error) {
    ...
    if (data.application?.id) {
      await updateApplication(env.apiBase, data.application.id, {
        ...
        notes: `LinkedIn assisted script could not continue safely: ${summary.errors.join("; ")}`
      });
    }
  } finally {
